...
router.get("/admin/slabs", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const resSlabs = await pool.query(
      `SELECT id, min_plots as "minPlots", max_plots as "maxPlots", monthly_price as "monthlyPrice", yearly_price as "yearlyPrice", status 
 ...
 */
router.post("/api/v1/admin/slabs", requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  const { id, minPlots, maxPlots, monthlyPrice, yearlyPrice, status } = req.body;
    ...
    if (id) {
      await pool.query(
        `UPDATE subscription_plot_slabs 
         SET min_plots = $1, max_plots = $2, monthly_price = $3, yearly_price = $4, status = $5, updated_at = CURRENT_TIMESTAMP
         WHERE id = $6`,
        [minPlots, maxPlots, monthlyPrice, yearlyPrice, status, id]
      );
    } else {
      await pool.query(
        `INSERT INTO subscription_plot_slabs (min_plots, max_plots, monthly_price, yearly_price, status)
         VALUES ($1, $2, $3, $4, $5)`,
        [minPlots, maxPlots, monthlyPrice, yearlyPrice, status || "ACTIVE"]
      );
    }
